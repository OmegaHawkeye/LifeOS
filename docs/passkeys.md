...
issue and authenticate native API tokens. LifeOS stores only the public key,
never the passkey's private key, and uses no hosted login relay or email flow.

## Server requirements

- Use HTTPS with a certificate trusted by each device. WebAuthn requires a
  secure context, except for local development on `localhost`.
- Give the home server a stable hostname such as `lifeos.home.arpa` and keep
  the same hostname after enrolling passkeys. Avoid a changing LAN IP;
  `localhost` on an iPhone refers to the iPhone, not the server.
- Set `APP_URL`, `LIFEOS_PASSKEY_WEB_URL`, and `LIFEOS_PASSKEY_WEB_ORIGIN` to
  the same LifeOS web URL, for example `https://lifeos.home.arpa`. The relying
  party ID is the hostname without scheme or port.
...
then returns a short-lived, one-time code protected with PKCE. The app
exchanges that code for its normal Sanctum access and refresh tokens.
