...
const navigation = [
  { to: "/dashboard", label: "Dashboard", shortLabel: "Today" },
  { to: "/finance", label: "Finance", shortLabel: "Finance" },
  { to: "/fitness", label: "Fitness", shortLabel: "Fitness" },
  { to: "/nutrition", label: "Nutrition", shortLabel: "Nutrition" },
  { to: "/health", label: "Health", shortLabel: "Health" },
    ...
    <div className="min-h-svh bg-stone-100 text-stone-950 dark:bg-stone-950 dark:text-stone-100">
      ...
      <div className="mx-auto flex max-w-[1600px] md:min-h-[calc(100svh-4rem)]">
        <nav
          ...
          className="fixed inset-x-0 bottom-0 z-30 flex gap-1 overflow-x-auto border-t border-stone-200 bg-white px-2 pb-[env(safe-area-inset-bottom)] pt-2 dark:border-white/10 dark:bg-stone-950 md:static md:w-56 md:shrink-0 md:flex-col md:gap-1 md:border-r md:border-t-0 md:bg-transparent md:px-4 md:py-8 md:pt-8 dark:md:bg-transparent"
        >
          {navigation.map((item, index) => (
            ...
            >
              <span
                aria-hidden="true"
                className="grid size-6 place-items-center rounded-lg bg-stone-200/70 text-xs dark:bg-white/5 md:size-7"
              >
                {index + 1}
              </span>
        ...
        <main className="min-w-0 flex-1 px-5 pb-28 pt-8 sm:px-8 md:px-10 md:pb-12 md:pt-10">
          {children}
        </main>
      </div>
    </div>
